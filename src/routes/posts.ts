...
posts.get('/:id', async (req,res) => {
  ...
  try{
    ...
    res.json(getFormattedPost(post, Number(req.query.latitude), Number(req.query.longitude)))
  } catch {
    res.send('bad id')
  }
})
...
posts.get('/', async (req, res) => {
  ...
  try {
    ...
    posts.forEach((post: IPost) => {
      stores.push(post.storename);
      const formattedPost = getFormattedPost(post, Number(req.query.latitude), Number(req.query.longitude))
      if(formattedPost.distance < 2000000){
        closestPosts.push(formattedPost);
      }
    })
    res.json({posts: sortPosts(closestPosts), stores})
  } catch {
